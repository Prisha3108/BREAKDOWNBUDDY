...
const FuelReply = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8000/fuel/getFuelRequests');
            if (response.status === 200) {
                setRequests(response.data);
            } else {
                console.error('Failed to fetch fuel requests');
                alert('Failed to fetch fuel requests');
            }
        } catch (error) {
            console.error('Error fetching fuel requests:', error);
            alert('Error fetching fuel requests');
        }
    };
    ...
    const handleApprove = async (requestId, index) => {
        if (window.confirm('Are you sure you want to approve this request?')) {
            try {
                const response = await axios.put(`http://localhost:8000/fuel/updateRequestStatus/${requestId}`, { status: 'Approved' });
                if (response.status === 200) {
    ...
    const handleDeny = async (requestId, index) => {
        if (window.confirm('Are you sure you want to deny this request?')) {
            try {
                const response = await axios.put(`http://localhost:8000/fuel/updateRequestStatus/${requestId}`, { status: 'Denied' });
                if (response.status === 200) {
                    const newRequests = [...requests];
