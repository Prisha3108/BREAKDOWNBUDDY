...
const BatteryReply = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8000/battery/getBatteryRequests');
            if (response.status === 200) {
                setRequests(response.data);
            } else {
                console.error('Failed to fetch battery requests');
                alert('Failed to fetch battery requests');
            }
        } catch (error) {
            console.error('Error fetching battery requests:', error);
            alert('Error fetching battery requests');
        }
    };
    ...
    const handleApprove = async (requestId, index) => {
        if (window.confirm('Are you sure you want to approve this request?')) {
            try {
                const response = await axios.put(`http://localhost:8000/battery/updateRequestStatus/${requestId}`, { status: 'Approved' });
                if (response.status === 200) {
    ...
    const handleDeny = async (requestId, index) => {
        if (window.confirm('Are you sure you want to deny this request?')) {
            try {
                const response = await axios.put(`http://localhost:8000/battery/updateRequestStatus/${requestId}`, { status: 'Denied' });
                if (response.status === 200) {
                    const newRequests = [...requests];
